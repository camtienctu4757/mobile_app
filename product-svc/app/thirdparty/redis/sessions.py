        ...
        try:
            async with self.get_connection() as conn:
                if self.__redis_method == "standalone":
                    ...
                    for i in clients:
                        if (
                            int(i["idle"]) > self.__redis_pool_timeout
                            and i["name"] == self.__redis_clientname
                            and i["user"] == self.__redis_username
                        ):
                            await conn.execute_command(
                                "CLIENT KILL", "ID", int(i["id"])
                            )
                    logger.debug(f"Idle connections was disconnected")
                elif self.__redis_method == "cluster":
                    nodes = [
                        i["host"]
                        for i in ast.literal_eval(get_env().REDIS_CLUSTER_HOSTS)
